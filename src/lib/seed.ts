...
async function insertWithId(client: DbClient, sql: string, params: unknown[]) {
  const id = randomUUID();
...
export async function seedDatabaseIfEmpty() {
  await transaction(async (client) => {
    const count = await client.query<{ count: number }>("select count(*) as count from estates");
    if (Number(count.rows[0].count) > 0) {
      return;
    }

    const estateId = await insertWithId(
      client,
    ...
    for (const user of users) {
      const id = await insertWithId(
        client,
        `insert into users (id, name, email, mobile, role, created_at) values (?,?,?,?,?,?)`,
    ...
    for (const period of billingPeriods) {
      const id = await insertWithId(
        client,
        `insert into billing_periods (id, estate_id, name, start_date, end_date, status, kwh_rate_pence, standing_charge_pence, levy_pence, created_by, issued_at, created_at)
         values (?,?,?,?,?,?,?,?,?,?,?,?)`,
    ...
    for (const payment of payments) {
      ...
      await client.query(
        `insert into payments (id, bill_id, unit_id, amount_pence, payment_method, payment_date, notes, recorded_by, created_at)
         values (?,?,?,?,?,?,?,?,?)`,
        [randomUUID(), billId, unitId, payment.amountPence, payment.paymentMethod, payment.paymentDate, payment.notes ?? null, userMap.get(payment.recordedBy) ?? null, payment.createdAt]
      );
    }
    ...
    for (const log of smsLogs) {
      ...
      if (!unitId) continue;
      await client.query(
        `insert into sms_logs (id, bill_id, unit_id, mobile, message, status, provider, provider_reference, sent_at, created_at)
         values (?,?,?,?,?,?,?,?,?,?)`,
        [randomUUID(), billId ?? null, unitId, log.mobile, log.message, log.status, log.provider, log.providerReference, log.sentAt ?? null, log.createdAt]
      );
    }
  });
}