...
function ModeSwitch({ rentMode, compact = false }: { rentMode: boolean; compact?: boolean }) {
  return <div className={`grid grid-cols-2 rounded-2xl border p-1 ${rentMode ? "border-amber-500/25 bg-amber-500/10" : "border-slateLine bg-card"} ${compact ? "w-56" : "w-full"}`}><Link href="/admin" className={`rounded-xl px-3 py-2 text-center text-xs font-black uppercase tracking-wide transition ${!rentMode ? "bg-estate-500 text-[#07110b]" : "text-secondaryText hover:bg-hover hover:text-ink"}`}>Utilities</Link><Link href="/admin/rent" className={`rounded-xl px-3 py-2 text-center text-xs font-black uppercase tracking-wide transition ${rentMode ? "bg-amber-400 text-[#161006] shadow-[0_0_22px_rgba(245,158,11,0.22)]" : "text-secondaryText hover:bg-hover hover:text-ink"}`}>Rent</Link></div>;
}

function ModeHeading({ rentMode }: { rentMode: boolean }) {
  return <section className={`rounded-2xl border p-4 ${rentMode ? "border-amber-500/30 bg-amber-500/10" : "border-estate-500/25 bg-estate-500/10"}`}><p className={`text-[11px] font-black uppercase tracking-[0.22em] ${rentMode ? "text-amber-300" : "text-estate-500"}`}>{rentMode ? "Rent Management" : "Utilities"}</p><p className="mt-1 text-sm font-bold text-secondaryText">{rentMode ? "Rent, arrears and unit payment tracking" : "Electric readings, bill cycle and utility payments"}</p></section>;
}
      ...
      <aside className="flex h-full w-[min(22rem,88vw)] flex-col border-r border-slateLine bg-sidebar p-4 shadow-2xl">
        ...
        <nav className="mt-5 grid gap-1 overflow-y-auto">
          {links.map(([label, href, Icon]) => {
            ...
            return <Link key={href} href={href} onClick={onClose} className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-bold ${active ? (rentMode ? "bg-amber-500/12 text-ink ring-1 ring-amber-500/35" : "bg-estate-500/12 text-ink ring-1 ring-estate-500/35") : "text-secondaryText hover:bg-hover hover:text-ink"}`}><Icon className={`h-5 w-5 ${active ? (rentMode ? "text-amber-300" : "text-estate-500") : "text-mutedText"}`} />{label}</Link>;
          })}
        </nav>
      </aside>
    </div>
...
export function AdminShell({ children, demoMode = false, billingPeriods = [] }: { children: ReactNode; demoMode?: boolean; billingPeriods?: BillingPeriod[] }) {
  ...
  return (
    <div className={`min-h-screen pb-24 text-ink lg:pb-0 ${rentMode ? "bg-[#11100d] bg-[radial-gradient(circle_at_top_right,rgba(245,158,11,0.12),transparent_34%),linear-gradient(135deg,#11100d_0%,#181510_48%,#101214_100%)]" : "bg-surface"}`}>
      ...
      {!landlordMode ? <aside className={`fixed left-0 top-0 hidden h-screen w-72 flex-col border-r border-slateLine p-5 lg:flex ${rentMode ? "border-amber-500/20 bg-[#14130f]" : "bg-sidebar"}`}>
        ...
        <nav className="mt-5 grid flex-1 gap-1.5 overflow-y-auto">{links.map(([label, href, Icon]) => { const active = isActive(pathname, href); return <Link key={href} href={href} className={`group flex items-center gap-3 rounded-2xl px-4 py-3.5 text-sm font-bold transition duration-200 ${active ? (rentMode ? "bg-amber-500/12 text-ink ring-1 ring-amber-500/35" : "bg-estate-500/12 text-ink ring-1 ring-estate-500/35") : "text-secondaryText hover:bg-hover hover:text-ink"}`}><Icon className={`h-5 w-5 ${active ? (rentMode ? "text-amber-300" : "text-estate-500") : rentMode ? "text-mutedText group-hover:text-amber-300" : "text-mutedText group-hover:text-estate-500"}`} /><span>{label}</span></Link>; })}</nav>
      ...
      <header className={`sticky top-0 z-10 border-b px-3 backdrop-blur-xl ${rentMode ? "border-amber-500/20 bg-[#12110e]/92" : "border-slateLine bg-surface/90"} ${landlordMode ? "py-2" : "py-3 lg:ml-72"}`}>
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {!rentMode ? <Link href={selectedPeriodId ? `/admin/readings?periodId=${selectedPeriodId}` : "/admin/readings"} className="tap-target hidden items-center justify-center gap-2 rounded-xl bg-estate-500 px-4 py-2.5 text-sm font-black text-[#07110b] shadow-glow transition hover:-translate-y-0.5 hover:bg-estate-600 sm:inline-flex"><Gauge className="h-5 w-5" />Enter Readings</Link> : <Link href="/admin/rent/checklist" className="tap-target hidden items-center justify-center gap-2 rounded-xl bg-amber-400 px-4 py-2.5 text-sm font-black text-[#161006] shadow-[0_0_24px_rgba(245,158,11,0.28)] transition hover:-translate-y-0.5 hover:bg-amber-300 sm:inline-flex"><HandCoins className="h-5 w-5" />Rent Checklist</Link>}
      ...
      {!landlordMode ? <nav className="fixed bottom-0 left-0 right-0 z-20 grid grid-cols-5 border-t border-slateLine bg-sidebar/95 backdrop-blur-xl lg:hidden">{links.slice(0, 5).map(([label, href, Icon, shortLabel]) => { const active = isActive(pathname, href); return <Link key={href} href={href} className={`flex flex-col items-center gap-1 py-3 text-[11px] font-bold ${active ? (rentMode ? "text-amber-300" : "text-estate-500") : "text-mutedText"}`}><Icon className="h-5 w-5" />{shortLabel}</Link>; })}</nav> : null}
    </div>
  );
}
