...
export function MeterReadingWorkspace({ svgMarkup, units, period, initialReadings, initialMode = "map" }: { svgMarkup: string; units: Unit[]; period: BillingPeriod; initialReadings: MeterReading[]; initialMode?: "map" | "list" }) {
  ...
  const filteredUnits = orderedUnits.filter((unit) => `${unit.unitReference} ${unit.tenantName}`.toLowerCase().includes(search.toLowerCase()));
  if (!selectedUnit || !previewBill) return <p className="text-secondaryText">No units are available for meter readings.</p>;
