...
export function FocusedMeterReadingMap({ svgMarkup, units, period, initialReadings }: { svgMarkup: string; units: Unit[]; period: BillingPeriod; initialReadings: MeterReading[] }) {
  ...
  if (orderedUnits.length === 0) return <div className="grid h-[100dvh] place-items-center bg-[#0b0d0f] p-6 text-ink"><section className="max-w-xl rounded-2xl border border-slateLine bg-card p-8 text-center shadow-soft"><BrandLogo className="mx-auto h-16 w-48 rounded-2xl" /><h1 className="mt-8 text-3xl font-black text-white">No units mapped</h1><p className="mt-3 font-bold text-mutedText">No database units match the visible yard map labels yet. Add units such as 1, 2/3, 4, 11A, 12A or FLAT 1.</p><Link href="/admin/units" className="tap-target mt-6 inline-flex items-center justify-center gap-2 rounded-2xl bg-estate-500 px-5 py-3 font-black text-[#07110b]">Open units</Link></section></div>;
