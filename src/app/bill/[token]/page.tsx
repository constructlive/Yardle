...
import { formatMoney, formatNumber, formatRoundedAccountBalance, formatRoundedMoney } from "@/lib/money";
...
export default async function PublicBillPage({ params }: { params: { token: string } }) {
  ...
  const displayBalancePence = latestBill ? roundedPayableBalance(latestBill.roundedTotalPence, latestBill.amountPaidPence) : 0;
  const paymentInstructions = latestBill
    ? renderPaymentInstructions(data.paymentInstructions, {
      ...
      amount: formatRoundedAccountBalance(displayBalancePence),
      paymentLink: ""
    })
    : "";
  return <main className="min-h-screen bg-canvas text-ink">
    ...
    <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 sm:py-12">
      {!latestBill || !latestPeriod ? <section className="rounded-2xl border border-slateLine bg-card p-8 text-center"><ReceiptText className="mx-auto h-10 w-10 text-estate-500" /><h1 className="mt-4 text-2xl font-black">No bills have been issued yet</h1></section> : <>
        ...
        <section className="mt-8 rounded-2xl border border-estate-500/30 bg-card p-6 shadow-glow sm:p-8"><p className="text-sm font-black uppercase text-secondaryText">{displayBalancePence < 0 ? "Account credit" : "Total due"}</p><p className="mt-2 text-5xl font-black sm:text-6xl">{formatRoundedAccountBalance(displayBalancePence)}</p><p className="mt-3 capitalize text-secondaryText">Current balance - {latestBill.paidStatus.replace("_", " ")}</p></section>
        <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Metric icon={<Gauge />} label="Units used" value={`${formatNumber(latestBill.usage)} kWh`} />
          <Metric icon={<WalletCards />} label="Current balance" value={formatRoundedAccountBalance(data.unit.currentBalancePence)} />
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[1.3fr_0.7fr]"><div className="rounded-2xl border border-slateLine bg-card p-6"><h2 className="text-xl font-black">Bill breakdown</h2><p className="mt-2 text-sm font-semibold leading-6 text-mutedText">This shows this month's electricity charges, then any unpaid balance or credit brought forward from earlier bills. Bill totals are shown rounded to the nearest pound.</p><dl className="mt-5 divide-y divide-slateLine"><Row label="Usage charge" value={formatRoundedMoney(latestBill.usageCostPence)} /><Row label="Standing charge" value={formatRoundedMoney(latestBill.standingChargePence)} /><Row label="Levy / other charges" value={formatRoundedMoney(latestBill.levyPence)} /><Row label="This month's charges" value={formatRoundedMoney(latestBill.subtotalPence)} /><Row label={broughtForwardLabel(latestBill.outstandingCarriedForwardPence)} value={formatRoundedAccountBalance(latestBill.outstandingCarriedForwardPence)} /><Row label="Bill total after brought forward balance" value={formatRoundedMoney(latestBill.roundedTotalPence)} /><Row label="Payments received" value={formatRoundedMoney(latestBill.amountPaidPence)} /><Row label={displayBalancePence < 0 ? "Credit remaining" : "Amount now due"} value={formatRoundedAccountBalance(displayBalancePence)} strong /></dl></div><aside className="rounded-2xl border border-slateLine bg-card p-6"><h2 className="text-xl font-black">Payment instructions</h2><InstructionText text={paymentInstructions} /><p className="mt-4 text-sm text-mutedText">Contact {data.estate.contactEmail}{data.estate.contactPhone ? ` or ${data.estate.contactPhone}` : ""}.</p></aside></section>
        ...
        <section className="mt-8"><h2 className="mb-4 text-xl font-black">Bill history</h2><div className="overflow-x-auto rounded-2xl border border-slateLine bg-card"><table className="w-full min-w-[52rem] text-left"><thead className="bg-sidebar text-sm text-secondaryText"><tr><th className="px-5 py-4">Period</th><th className="px-5 py-4">This month</th><th className="px-5 py-4">Brought forward</th><th className="px-5 py-4">Paid</th><th className="px-5 py-4">Balance</th><th className="px-5 py-4">Status</th></tr></thead><tbody className="divide-y divide-slateLine">{data.bills.map((bill) => { const period = data.billingPeriods.find((item) => item.id === bill.billingPeriodId); return <tr key={bill.id} className="hover:bg-hover"><td className="px-5 py-4 font-bold">{period?.name ?? "-"}</td><td className="px-5 py-4">{formatRoundedMoney(bill.subtotalPence)}</td><td className="px-5 py-4">{formatRoundedAccountBalance(bill.outstandingCarriedForwardPence)}</td><td className="px-5 py-4">{formatRoundedMoney(bill.amountPaidPence)}</td><td className="px-5 py-4 font-black">{formatRoundedAccountBalance(roundedPayableBalance(bill.roundedTotalPence, bill.amountPaidPence))}</td><td className="px-5 py-4 capitalize">{bill.paidStatus.replace("_", " ")}</td></tr>; })}</tbody></table></div></section>
      </>}
