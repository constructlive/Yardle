...
type Filter = "all" | "paid" | "unpaid" | "part_paid" | "credited" | "outstanding";
...
const filters: Array<[Filter, string]> = [["all", "All"], ["paid", "Paid"], ["unpaid", "Unpaid"], ["part_paid", "Part Paid"], ["credited", "Credited"], ["outstanding", "Outstanding only"]];
...
function statusForPayment(amountPaidPence: number, totalDuePence: number): PaidStatus { const balance = totalDuePence - amountPaidPence; return balance < 0 ? "credited" : balance === 0 ? "paid" : amountPaidPence > 0 ? "part_paid" : "unpaid"; }
...
function statusClass(status: PaidStatus) { if (status === "paid") return "bg-estate-500/15 text-estate-500"; if (status === "credited") return "bg-blue-500/15 text-blue-300"; if (status === "part_paid") return "bg-amber-500/15 text-amber-300"; return "bg-red-500/15 text-red-300"; }
...
export function LandlordPaymentView({ period, rows, initialFilter = "all" }: { period?: BillingPeriod; rows: LandlordRow[]; initialFilter?: Filter }) {
  ...
  const summary = useMemo(() => ({
    totalOutstanding: payments.reduce((sum, row) => sum + Math.max(0, row.remainingBalancePence), 0),
    totalCredit: payments.reduce((sum, row) => sum + Math.max(0, -row.remainingBalancePence), 0),
    ...
    credited: payments.filter((row) => row.paidStatus === "credited").length
  }), [payments]);
  ...
  function confirmPayment() {
    ...
    const desiredAmountPaidPence = paymentModal.amountPaidPence + paymentAmountPence;
    ...
    const row = { ...paymentModal, amountPaidPence: desiredAmountPaidPence, remainingBalancePence: paymentModal.totalDuePence - desiredAmountPaidPence, paidStatus: statusForPayment(desiredAmountPaidPence, paymentModal.totalDuePence) };
  ...
  function exportCsv() {
    const header = ["unit", "tenant", "total due", "amount paid", "balance", "status"];
