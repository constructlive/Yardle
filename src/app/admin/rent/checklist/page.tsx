  ...
  return <>
    ...
    <section className="mb-5 rounded-2xl border border-slateLine bg-card p-5 shadow-soft">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div><p className="text-sm font-black uppercase tracking-wide text-estate-500">Today&apos;s position</p><h2 className="mt-1 text-3xl font-black text-ink">{formatMoney(outstanding)} outstanding</h2><p className="mt-1 font-bold text-mutedText">Generated rent charges minus recorded payments.</p></div>
    ...
    <section className="overflow-x-auto rounded-2xl border border-slateLine bg-card shadow-soft">
      <table className="min-w-[78rem] w-full divide-y divide-slateLine text-left text-sm [&_tbody_tr:nth-child(even)]:bg-white/[0.025] [&_tbody_tr:hover]:bg-hover">
        <thead className="sticky top-0 z-20 bg-sidebar text-secondaryText"><tr><th className="px-4 py-4">Status</th><th className="px-4 py-4">Unit</th><th className="px-4 py-4">Tenant</th><th className="px-4 py-4">Rent rule</th><th className="px-4 py-4">Charged</th><th className="px-4 py-4">Paid</th><th className="px-4 py-4">Balance</th><th className="px-4 py-4">Record payment</th></tr></thead>
        <tbody className="divide-y divide-slateLine">{ledger.length ? ledger.map((row) => <tr key={row.unit.id}><td className="px-4 py-4"><StatusPill tone={rentStatusTone(row.status)}>{rentStatusLabel(row.status)}</StatusPill></td><td className="px-4 py-4 text-xl font-black text-ink">{row.unit.unitReference}</td><td className="px-4 py-4 font-bold text-secondaryText">{row.unit.tenantName || "Vacant"}</td><td className="px-4 py-4 text-secondaryText">{formatMoney(row.weeklyOrMonthlyRentPence)}<br /><span className="text-xs font-bold text-mutedText">{rentFrequencyLabel(row.frequency)} · next {row.nextDueDate}</span></td><td className="px-4 py-4 font-black text-ink">{formatMoney(row.chargedPence)}</td><td className="px-4 py-4 font-black text-ink">{formatMoney(row.paidPence)}</td><td className={`px-4 py-4 font-black ${row.balancePence > 0 ? "text-amber-400" : row.balancePence < 0 ? "text-blue-300" : "text-estate-500"}`}>{formatAccountBalance(row.balancePence)}</td><td className="px-4 py-4"><form action={saveRentPayment} className="grid min-w-[28rem] grid-cols-[1fr_1fr_auto] gap-2"><input type="hidden" name="unitId" value={row.unit.id} /><input name="amount" defaultValue={(Math.max(0, row.balancePence) / 100).toFixed(2)} inputMode="decimal" className="h-12 rounded-xl border border-slateLine bg-sidebar px-3 text-base font-black text-ink outline-none focus:border-estate-500" /><select name="paymentMethod" defaultValue="bank_transfer" className="h-12 rounded-xl border border-slateLine bg-sidebar px-3 font-black text-ink outline-none focus:border-estate-500"><option value="bank_transfer">Bank Transfer</option><option value="cash">Cash</option><option value="card">Card</option><option value="other">Other</option></select><button className="inline-flex h-12 items-center gap-2 rounded-xl bg-estate-500 px-4 font-black text-[#07110b]"><CheckCircle2 className="h-5 w-5" />Record</button><input type="date" name="paymentDate" defaultValue={todayIso()} className="h-12 rounded-xl border border-slateLine bg-sidebar px-3 font-bold text-ink outline-none focus:border-estate-500" /><input name="notes" placeholder="Notes" className="col-span-2 h-12 rounded-xl border border-slateLine bg-sidebar px-3 font-bold text-ink outline-none focus:border-estate-500" /></form></td></tr>) : <tr><td colSpan={8} className="px-4 py-10 text-center font-bold text-mutedText">No rent units are configured yet. Open Unit Rent Settings to enable rent tracking.</td></tr>}</tbody>
      </table>
    </section>
  </>;
}