...
export default async function RentArrearsPage() {
  ...
  return <>
    ...
    <DataTable><thead><tr><Th>Unit(s)</Th><Th>Tenant / account</Th><Th>Rent rule</Th><Th>Opening</Th><Th>Generated</Th><Th>Paid</Th><Th>Outstanding</Th><Th>Status</Th></tr></thead><tbody>{ledger.length ? ledger.map((row) => <tr key={row.memberUnitIds.join("-")}><Td strong>{row.unitReferences}</Td><Td>{row.tenantName}</Td><Td>{formatMoney(row.weeklyOrMonthlyRentPence)} {rentFrequencyLabel(row.frequency)}</Td><Td>{formatAccountBalance(row.openingBalancePence)}</Td><Td>{formatMoney(row.chargedPence)}</Td><Td>{formatMoney(row.paidPence)}</Td><Td strong>{formatAccountBalance(row.balancePence)}</Td><Td><StatusPill tone={rentStatusTone(row.status)}>{rentStatusLabel(row.status)}</StatusPill></Td></tr>) : <tr><td colSpan={8} className="px-4 py-10 text-center font-bold text-mutedText">No rent arrears at the moment.</td></tr>}</tbody></DataTable>
  </>;
}
